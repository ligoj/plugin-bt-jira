...
public class JiraBaseResource {
	 ...
	 * @return the data source of JIRA database server.
	 */
	public DataSource getDataSource(final int subscription) throws InstantiationException, IllegalAccessException, ClassNotFoundException {
	 ...
	 * 
	 * @param parameters
	 *            the subscription parameters containing at least the data source configuration.
	 * @return the data source of JIRA database server.
	 */
	protected DataSource getDataSource(final Map<String, String> parameters)
			throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		return new SimpleDriverDataSource(
				(Driver) Class.forName(StringUtils.defaultIfBlank(parameters.get(PARAMETER_JDBC_DRIVER), "com.mysql.jdbc.Driver")).newInstance(),
	 ...
	 */
	protected String validateDataBaseConnectivity(final Map<String, String> parameters) throws Exception {
	 ...
	 */
	protected boolean validateAdminConnectivity(final Map<String, String> parameters) throws Exception {
	 ...
	 */
	protected boolean authenticateAdmin(final Map<String, String> parameters, final CurlProcessor processor) throws Exception {
		...
		requests.add(new CurlRequest(HttpMethod.POST, url,
				"os_username=" + user + "&os_password=" + password + "&os_destination=&atl_token=&login=Connexion", JiraCurlProcessor.LOGIN_CALLBACK,
				"Accept:text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"));

		// Sudoing is only required for JIRA 4+
		if ("4".compareTo(getVersion(parameters)) <= 0) {
			requests.add(new CurlRequest(HttpMethod.POST, StringUtils.appendIfMissing(baseUrl, "/") + "secure/admin/WebSudoAuthenticate.jspa",
					"webSudoIsPost=false&os_cookie=true&authenticate=Confirm&webSudoPassword=" + password, JiraCurlProcessor.SUDO_CALLBACK,
					"Accept:text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"));
		}
	...
	/**
	 * Update the status, priorities, resolutions and types text of given SLA and retrieve all status texts involved of
	 * issues of
	 * given project.
	 * 
	 * @param dataSource
	 ...
	 * @return a {@link Map} where KEY is the status identifier and the VALUE is the status name, upper case and
	 *         un-localized.
	 */
	...
	/**
	 * Return workflow steps of given workflow's name and managing default 'jira' workflow. KEY is the status, VALUE is
	 * the corresponding workflow's step
	 */
	private Workflow getWorkflow(final String name, final String workflowXml, final Map<Integer, String> statuses) throws IOException {
		if (workflowXml == null) {
			// Default worflow
	...
	/**
	 * Return type to status to step mapping for all types valid for given project. KEY is the type, VALUE is the
	 * workflow mapping status and steps.
	 * 
	 * @param dataSource
	 *            The data source of JIRA database.
	 * @param jira
	 ...
	 */
	protected Map<Integer, Workflow> getTypeToStatusToStep(final DataSource dataSource, final int jira, final Map<Integer, String> statuses)
			throws IOException {
	...
	/**
	 * Return workflow steps of given workflow's name. KEY is the status, VALUE is the corresponding workflow's step
	 */
	private Map<String, INamableBean<Integer>> getWorkflowSteps(final String workflowXml, final Map<Integer, String> statuses) throws IOException {
